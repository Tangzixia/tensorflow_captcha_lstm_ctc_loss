...
if __name__=="__main__":
    ...
    indices_=tf.placeholder(dtype=tf.int64,shape=[None,2])
    shape_ = tf.placeholder(dtype=tf.int64, shape=[2])
    #targets = tf.sparse_placeholder(tf.int32)
    targets = tf.SparseTensor(indices=indices_, values=values_, dense_shape=shape_)
    ...
    with tf.Session() as sess:
        ...
        for i in range(20000):
            ...
            for i_ in range(len(batches_indices_initial)):
                ...
                per_acc+=train_acc
                per_loss+=train_loss
            print(per_acc,len(batches_dense_indices_initial))
